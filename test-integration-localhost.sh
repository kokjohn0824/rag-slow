...
# ============================================================================
# 完整整合測試：Anomaly Service + Source Code API (Localhost 版本)
# ============================================================================
# 
# 此腳本用於在本地環境測試 Anomaly Service 的 child-span-anomalies API
#
# 必要服務 (透過 docker compose 啟動):
#   - tempo-otlp-trace-demo: Tempo, OTel Collector, Grafana, trace-demo-app
#   - tempo-latency-anomaly-service: Anomaly Service, Redis
#
# 啟動服務:
#   cd tempo-otlp-trace-demo && docker compose up -d
#   cd tempo-latency-anomaly-service && make docker-up
#
# ============================================================================
...
# Localhost 配置
ANOMALY_SERVICE="${ANOMALY_SERVICE:-http://localhost:8081}"
TRACE_DEMO_SERVICE="${TRACE_DEMO_SERVICE:-http://localhost:8080}"
TEMPO_SERVICE="${TEMPO_SERVICE:-http://localhost:3200}"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo ""
      echo "環境變數:"
      echo "  ANOMALY_SERVICE         Anomaly Service URL (預設: http://localhost:8081)"
      echo "  TRACE_DEMO_SERVICE      Trace Demo Service URL (預設: http://localhost:8080)"
      echo "  TEMPO_SERVICE           Tempo URL (預設: http://localhost:3200)"
...
if [ "$SERVICES_OK" = false ]; then
  ...
  echo ""
  echo "請確保以下服務已啟動:"
  echo "  1. cd tempo-otlp-trace-demo && docker compose up -d"
  echo "  2. cd tempo-latency-anomaly-service && make docker-up"
  echo ""
  exit 1
fi
